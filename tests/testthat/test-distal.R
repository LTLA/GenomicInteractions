...
test_that("identification of distal anchors works correctly", {
    x <- spawn_gi()
    combined <- unlist(regions(x, type=NULL))
    ...
    for (i in seq_along(combined)) {
        ...
        expect_identical(distals, ref[order(o)])
    }
})
...
test_that("first anchors go before second anchors when local=TRUE", {
    ...
    expect_identical(out, c(first(x), second(x)))
})
...
test_that("findDistalAnchors passes arguments along correctly", {
    ...
    o <- c(which(olap2), which(olap1))
    ref <- c(first(x)[olap2], second(x)[olap1])
    expect_identical(distal1, ref[order(o)])
...
test_that("findDistalAnchors handles metadata correctly", {
    ...
    mcols(featureSets(x)[[1]])$thing <- 10
    ...
    mcols(featureSets(x)[[2]])$thing <- 20
    expect_warning(out <- findDistalAnchors(x, thing), NA)

    olap1 <- overlapsAny(first(x), thing)
    olap2 <- overlapsAny(second(x), thing)
    o <- c(which(olap2), which(olap1))
    expect_identical(out$thing, c(first(x)$thing[olap2], second(x)$thing[olap1])[order(o)])
...
test_that("linearization behaves correctly with empty inputs", {
    x <- spawn_gi()
    expect_identical(length(findDistalAnchors(x[0,], first(x)[1])), 0L)

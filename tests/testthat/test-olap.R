...
CHECK_1.5D_HITS <- function(x, y, ...) {
    olap11 <- findOverlaps(first(x), first(y), ...)
    olap12 <- findOverlaps(first(x), second(y), ...)
    olap21 <- findOverlaps(second(x), first(y), ...)
    olap22 <- findOverlaps(second(x), second(y), ...)

    expect_identical_hits(findOverlaps(x, y, ..., use.region="first-first"), olap11)
    expect_identical_hits(findOverlaps(x, y, ..., use.region="first-second"), olap12)
    expect_identical_hits(findOverlaps(x, y, ..., use.region="second-first"), olap21)
    expect_identical_hits(findOverlaps(x, y, ..., use.region="second-second"), olap22)

    expect_identical_hits(findOverlaps(x, y, ..., use.region="first-any"), union(olap11, olap12))
    expect_identical_hits(findOverlaps(x, y, ..., use.region="any-first"), union(olap11, olap21))
    expect_identical_hits(findOverlaps(x, y, ..., use.region="second-any"), union(olap21, olap22))
    expect_identical_hits(findOverlaps(x, y, ..., use.region="any-second"), union(olap12, olap22))

    expect_identical_hits(findOverlaps(x, y, ..., use.region="any-any"), Reduce(union, list(olap11, olap12, olap21, olap22)))
}

test_that("findOverlaps works with 1.5D overlaps", {
    x <- spawn_gi()
    y <- spawn_gi()

    CHECK_1.5D_HITS(x, y)
    CHECK_1.5D_HITS(x, y, type="within")
    CHECK_1.5D_HITS(x, y, maxgap=20)
    CHECK_1.5D_HITS(x, y, minoverlap=10)
...
test_that("findOverlaps works with 2D overlaps", {
    ...
    CHECK_1.5D_HITS(x, y, ignore.strand=FALSE)
    CHECK_1.5D_HITS(x, y, ignore.strand=FALSE, type="within")
    CHECK_1.5D_HITS(x, y, ignore.strand=FALSE, maxgap=20)
    CHECK_1.5D_HITS(x, y, ignore.strand=FALSE, minoverlap=10)
})

test_that("2D overlaps behave with empty inputs", {
    x <- spawn_gi()
    y <- spawn_gi()
    CHECK_1.5D_HITS(x[0], y)
    CHECK_1.5D_HITS(x, y[0])
    CHECK_1.5D_HITS(x[0], y[0])
})

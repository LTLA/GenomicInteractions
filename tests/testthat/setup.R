...
spawn_gi <- function(N1=30, N2=15, Np=50) {
    ...
    regions1 <- GRanges(sample(chromos, N1, replace=TRUE), IRanges(all.starts, all.ends),
        seqlengths=c(chrA=1000, chrB=2000))

    all.starts <- round(runif(N2, 1, 100))
    all.ends <- all.starts + round(runif(N2, 5, 20))
    regions2 <- GRanges(sample(chromos, N2, replace=TRUE), IRanges(all.starts, all.ends),
        seqlengths=c(chrA=1000, chrB=2000))
...
spawn_regions <- function(Ngenes=10) {
    ...
    GRanges(sample(chromos, Ngenes, replace=TRUE), IRanges(gene.starts, gene.ends), strand=gene.strand)
}

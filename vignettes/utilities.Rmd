...
Different types of distances are obtained by specifying the `type` argument, e.g., `"gap"`, `"span"`, `"diagonal"`.
...
```{r}
gi2 <- gi
seqlevels(first(gi2)) <- "chrB"
try(boundingBox(gi, gi2))
```
...
index.1 <- c(5, 3, 3, 1, 9, 10)
index.2 <- c(1, 5, 6, 9, 7,  4) 
gi <- GenomicInteractions(index.1, index.2, all.regions)
gi
swapAnchors(gi)
```
...
```{r}
all.regions <- GRanges("chrA", IRanges(1:10*10, width=10))
index.1 <- c(1, 8, 10, 2, 6, 2, 1)
index.2 <- c(9, 5,  4, 5, 4, 8, 2)
gi <- GenomicInteractions(index.1, index.2, all.regions)
gi
```

...
Interaction-specific metadata can be stored via the `mcols` replacement method or through the `$` wrapper.
...
Note that this differs from the region-specific metadata that was set using `regions()<-` above.
...
Details for any given method can be found through the standard methods, 
